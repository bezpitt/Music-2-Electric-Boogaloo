function [soundSample]= create_sound(instrument,notes, constants)
    ...
    [notechooser,octavechooser] = split(notes.note, ["0", "1", "2", "3", "4", "5", "6", "7"]);
    ...
    switch instrument.sound
        ...
        case {'Subtractive'}
            ...
            for k = 1+filterLength:Length(2)+filterLength-1
                [b,a] = zp2tf([1],Z(k-filterLength,:),1);
                y(k) = Square(k)- a(2)*y(k-1) - a(3)*y(k-2);
            end
            soundSample = y;
        ...
        case {'FM'}
            ...
            soundSample = F1.* sin(2*pi*(startnote + FM_SIN).*t);
        case {'Waveshaper'}
